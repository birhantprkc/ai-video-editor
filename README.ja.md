...
- **2026年9月8日 — 再生ヘッドのドラッグを滑らかに：** ポインター更新を表示フレームごとにまとめ、処理中の動画シークを完了してから最新の指定時刻へ進めます。タイムラインのサンプリング結果も再利用します。ドラッグ中はサムネイルの精細化を停止し、離した位置へ正確に移動します。操作方法は変わりません。
- **2026年9月8日 — エージェント向けタイムラインマーカー：** Skill、CLI、MCP からマーカーの読み取り・追加・更新・削除ができ、チャプターや音楽のビートの計画、修正メモの記録に使えます。変更を適用する前にプロジェクトを確認し、意味的な差分をプレビューできます。
...
検証済みのリリースに固定するには `--pin v1.0.7` を追加します。インストール前の確認には `gh skill preview MartinDelophy/ai-video-editor edit-timeline-studio` を使用してください。
